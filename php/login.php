...
if ($num == 1) {
                    if ($hash == $hpass) {
                              ...
                              if ($authsub=="subscribed") {
                                        ...
                                        echo '<script>document.cookie = "usercookie={isSUB:true}";</script>';
                              }
                              else{
                                        // $_SESSION['auth-lvl'] = '{isSUB:false}';
                                        echo '<script>document.cookie = "usercookie={isSUB:false}";</script>';
                              }
                              ...
                              echo '<script>alert("Error: You have entered incorrect username or password."); location.href="/login.html";</script>';
                    }
          }
else {
          echo '<script>alert("Opps: fill the data for login."); location.href="/login.html";</script>';
}
?>